...
<div class="container">
    ...
    @endif
    <form action="{{ route('empleados.store') }}" class="mt-4 d-flex flex-column gap-3" method="POST" id="empleadoForm">
        @csrf
        <div class="form-group d-flex gap-5">
            ...
            <div class="container p-0">
                <input type="text" value="{{old('nombre')}}" name="nombre" placeholder="Nombre completo del empleado"
        ...
        <div class="form-group d-flex gap-5">
            ...
            <div class="container p-0">
                <input type="email" value="{{old('email')}}" name="email" placeholder="Correo electrónico"
                    class="form-control">
                @error('email')
                <span class="text-danger">{{$message}}</span>
                @enderror
            ...
            <div class="container p-0">
                <div class="d-flex flex-column gap-1">
                    <div class="d-flex align-items-center gap-3">
                        <input type="radio" name="sexo" value="M" id="M" class="form-check-input" {{ old('sexo')=='M'
                            ? 'checked' : '' }}>
                        <label for="M" class="form-check-label">Masculino</label>
                    </div>
                    <div class="d-flex align-items-center gap-3">
                        <input type="radio" name="sexo" value="F" id="F" class="form-check-input" {{ old('sexo')=='F'
                            ? 'checked' : '' }}>
        ...
        <div class="form-group d-flex gap-5">
            ...
            <div class="container p-0">
                <textarea name="descripcion" placeholder="Descripción de la experiencia del empleado" class="form-control">{{ old('descripcion') }}</textarea>
                @error('descripcion')
                <span class="text-danger">{{$message}}</span>
                @enderror
            </div>
            ...
            <div class="container p-0">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="roles[]" value="{{ $rol->id }}"
                        id="rol{{ $rol->id }}" {{ in_array($rol->id, old('roles', [])) ? 'checked' : '' }}>
